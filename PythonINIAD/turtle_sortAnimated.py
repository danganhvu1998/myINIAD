...
def merge(shelf, h1, h2, startPos):
    h1Pos = h2Pos = rPos = 0
    n = len(h1)+len(h2)
    r = [None]*n
    print("--------->>>>>>>", n, h1,h2)
    for i in range(0,n):
        if(h1Pos == len(h1)):
            ...
            if(h1[h1Pos].size  < h2[h2Pos].size):
                r.append( h1[h1Pos] )
                h1Pos+=1
            else:
                r.append( h2[h2Pos] )
                h2Pos+=1
        rPos+=1
    visualize_merge(shelf, r, startPos)

def msort(shelf,startPos, endPos):
    if(startPos == endPos):
        return shelf[startPos:endPos+1]
    l  = endPos - startPos + 1
    h1 = msort(shelf, startPos, startPos+l//2-1)
    h2 = msort(shelf, startPos+l//2, endPos)
    return merge(shelf, h1, h2, startPos)
...
instructions1 = "press i for insertion sort, s for selection sort, q for quicksort, m for MergeSort"
