...
int main(int argc, char *argv[])
{
    ...
    net2_nodes.Add(net1_nodes.Get(1)); // n0 is set as shared node
    net2_nodes.Create(1);              // create n2 node

    NodeContainer net3_nodes;
    net3_nodes.Add(net2_nodes.Get(1)); // n0 is set as shared node
    net3_nodes.Create(1);              // create n3 node

    NodeContainer net4_nodes;
    net4_nodes.Add(net3_nodes.Get(1));
    net4_nodes.Create(1);

    NodeContainer net5_nodes;
    net5_nodes.Add(net4_nodes.Get(1));
    net5_nodes.Create(1);

    NodeContainer net6_nodes;
    net6_nodes.Add(net5_nodes.Get(1));
    net6_nodes.Create(1);

    NodeContainer net7_nodes;
    net7_nodes.Add(net6_nodes.Get(1));
    net7_nodes.Add(net6_nodes.Get(0));
    ...
    devices7 = p2p1.Install(net7_nodes);
    ...
    address.SetBase(NET_ADD7, NET_MASK, FIRST_NO);
    Ipv4InterfaceContainer ifs7 = address.Assign(devices7);
    ...
    ApplicationContainer sourceApp1 = ftp4.Install(net1_nodes.Get(0));
    ...
    // attach ftp to source node n1
    ApplicationContainer sourceApp2 = ftp5.Install(net2_nodes.Get(0));
    ...
    // attach ftp to source node n1
    ApplicationContainer sourceApp3 = ftp6.Install(net3_nodes.Get(0));
    ...
    // for PCAP trace
    std::string pfname = std::string(PROG_DIR) + "circleNetwork-";
    p2p1.EnablePcap(pfname + "node1", devices1.Get(1), true);
    p2p1.EnablePcap(pfname + "node2", devices2.Get(1), true);
    p2p1.EnablePcap(pfname + "node3", devices3.Get(1), true);
